...
class AVLTree:
    ...
    # 인접한 블록을 찾기
    def find_adjacent(self, start_address):
        return self.__findAdjacentBlocks(self.__root, start_address)

    def __findAdjacentBlocks(self, tNode, start_address):
        if tNode == self.NIL:
            return None, None

        left_adjacent, right_adjacent = None, None

        if tNode.start_address + tNode.item == start_address:
            left_adjacent = tNode
        elif start_address + tNode.item == tNode.start_address:
            right_adjacent = tNode

        if left_adjacent is None:
            left_adjacent = self.__findAdjacentBlocks(tNode.left, start_address)[0]
        if right_adjacent is None:
            right_adjacent = self.__findAdjacentBlocks(tNode.right, start_address)[1]
...
class Allocator:
    ...
    # 인접 블록 병합
    def merge_adjacent_free_blocks(self, start_address, size):
        left_adjacent, right_adjacent = self.free_list.find_adjacent(start_address)

        if left_adjacent:
            size += left_adjacent.item
            start_address = left_adjacent.start_address
            self.free_list.delete(left_adjacent.item)

        if right_adjacent:
            size += right_adjacent.item
            self.free_list.delete(right_adjacent.item)
    ...
    def free(self, id):
        ...
        if node:  # 블록을 찾은 경우
            self.merge_adjacent_free_blocks(node.start_address, node.size)  # 인접한 자유 블록 병합
